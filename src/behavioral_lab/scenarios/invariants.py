...
def validate_invariants(world: WorldState, total_food: int = 20) -> None:
    ...
    trackable_food = world.remaining_food + sum(a.inventory for a in world.agents.values())
    if trackable_food < 0 or trackable_food > total_food:
        raise AssertionError("trackable food is outside the initial total")
